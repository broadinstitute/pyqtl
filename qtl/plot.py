...
def setup_figure(aw=4.5, ah=3, xspace=[0.75,0.25], yspace=[0.75,0.25],
                 colorbar=None, ds=0.15, cw=0.12, ct=0, ch=None, facecolor='none',
    ...
    if margin_axes in ['y', 'both']:
        fh += dy + my
    fig = plt.figure(facecolor=facecolor, figsize=(fw,fh))
...
def shared_x_label(axes, xlabel, default_offset=0.04, fontsize=12):
    fig = axes[0].figure
    fig.canvas.draw()
    ...
    if label_bboxes:
        # transform to figure coordinates
        bboxes_fig = [bbox.transformed(fig.transFigure.inverted()) for bbox in label_bboxes]
        y0 = min(bbox.y0 for bbox in bboxes_fig)  # select lowest
    else:
        bbox = np.vstack([ax.get_position().bounds for ax in axes])
        y0 = bbox[:, 1].min() - default_offset

    bbox = np.vstack([ax.get_position().bounds for ax in axes])
    x0 = bbox[:, 0].min()
    x1 = (bbox[:, 0] + bbox[:, 2]).max()
    x_center = (x0 + x1) / 2
    fig.text(x_center, y0, xlabel, fontsize=fontsize, va="top", ha="center")
...
class CohortLabel(object):
    ...
    def plot(self, ix=None, ax=None, cax=None, show_frame=False, rasterized=False, colorbar=False,
             cbar_orientation='horizontal', cbar_position='relative', cdl=0.25, cdt=0):
        if ax is None:
        ...
        if self.cohort_s.dtype.name == 'category':
            ...
            if cax is None:  # add colorbar axes
                fig = ax.figure
                ...
                if cbar_orientation == 'horizontal':
                    if cbar_position == 'relative':
                        cax = fig.add_axes([bbox.x1 + cdl/fw, bbox.y0-cdt/fh, 0.5/fw, bbox.height])  # horizontal
                    else:
                        cax = fig.add_axes([cdl/fw, (fh-cdt)/fh, 0.5/fw, bbox.height])  # horizontal
                else:
